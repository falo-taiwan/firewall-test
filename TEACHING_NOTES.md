...
## 單元九：顧問現場調試與內網穿透技術 (Port Forwarding & Tunneling)

在實際的售前與企業訪談中，顧問常常需要在客戶的防火牆環境中展示本地修改的網頁，或是將本地啟動的測試伺服器臨時暴露給外網的客戶進行測試。由於本專案為 100% 靜態網頁，本單元將深入介紹並整理市面上主流的「轉 Port / 內網穿透」服務，並探討其在企業資安環境下的實戰價值。

### 1. 內網穿透技術原理 (How Tunneling Works)
內網穿透服務（Reverse Tunneling）本質上是在本地客戶端（Client）與公網伺服器（Server）之間建立一條安全的加密通道（Tunnel）。
* 當外部訪客存取公網伺服器分配給您的專屬網址時，伺服器會將該 HTTP 流量透過已建立的通道「轉送」至您本地電腦的指定 Port（例如 `http://localhost:8000`）。
* 這使得運行在 NAT（網路位址轉換）或防火牆後端的本地服務，在不配置路由器 Port Forwarding 的情況下也能被公網直接存取。

### 2. 9 大主流轉 Port / 內網穿透服務全收集

為此，我們全面收集並整理了目前市場上最受歡迎的內網穿透方案，幫助顧問在不同企業限制下靈活選用：

#### A. Cloudflare Tunnel (cloudflared) -【企業級首選，最穩定】
* **介紹**：Cloudflare 提供的安全穿透服務。
* **優勢**：完全免費，且**沒有訪客警告頁面**。極高的傳輸穩定度，且支援防護 DDoS。如果顧問有自訂網域並託管在 Cloudflare，可綁定為固定的企業域名。
* **劣勢**：初次配置需要註冊 Cloudflare 帳戶；若要固定網域，需要設定 DNS 代管。
* **快速命令 (臨時隨機網域)**：
  ```bash
...
* **介紹**：全球開發者最常用的內網穿透工具。
* **優勢**：成熟穩定，指令極簡，全球節點多。
* **劣勢**：免費版每次啟動皆隨機生成網址、有連線速度與頻寬限制。最關鍵的是，**訪客存取時會先跳出「ngrok 安全警告頁面」**，需要點擊 "Visit Site" 才能進入，這對不熟悉技術的企業客戶會造成資安疑慮。
* **快速命令**：
  ```bash
  ngrok http 8000
  ```

#### C. Localtunnel -【最快捷的 Node.js 工具】
* **介紹**：以 Node.js 寫成的開源穿透工具。
* **優勢**：完全免費，免註冊帳號、免安裝本地二進位檔。**支援自訂子網域前綴**（例如：`--subdomain falo-test`）。
* **劣勢**：國外伺服器有時連線較慢，在大流量或多連線時容易瞬斷。
* **快速命令**：
  ```bash
  npx localtunnel --port 8000 --subdomain falo-test
  ```

#### D. VS Code Built-in Port Forwarding -【開發者無縫整合】
* **介紹**：VS Code 內建的埠轉送功能。
* **優勢**：完全不需要安裝額外 CLI 工具，在編輯器內 "Ports" 面板中新增 Port 即可。可設定連接埠的存取權限（Private / Public）。
* **劣勢**：生成的連結格式為 `*.github.dev`，有時需要存取者登入 GitHub 帳戶驗證，不適合直接傳給非開發人員的客戶。

#### E. frp (Fast Reverse Proxy) -【自建私有通道首選】
* **介紹**：一款高性能的開源反向代理應用，支援 TCP/UDP/HTTP/HTTPS。
* **優勢**：100% 自主控管。可配置在企業自己的 VPS（例如阿里雲、AWS EC2）上，數據完全不流經第三方平台，安全合規。
* **劣勢**：需要自備公網 IP 伺服器，且需手動配置 `frps.ini` 與 `frpc.ini`，運維成本較高。

#### F. Bore -【現代 Rust 輕量穿透】
* **介紹**：使用 Rust 語言編寫的極簡內網穿透工具，設計目的為「Just work」。
* **優勢**：極度輕量。官方提供免費的公共實例 `bore.pub`，使用非常直覺，單一檔案即可執行。
* **劣勢**：主要用於 TCP 端口轉發，若要處理 HTTP 虛擬主機頭部（Host Header）或 HTTPS 證書，需要另外搭配 Nginx。
* **快速命令**：
  ```bash
  bore local 8000 --to bore.pub
  ```

#### G. SSH Reverse Tunneling (serveo.net / localhost.run) -【零安裝客戶端黑科技】
* **介紹**：利用系統內建的 SSH 客戶端與公網 SSH 伺服器建立反向通道。
* **優勢**：**本機完全不需要下載任何軟體**，只要電腦有 `ssh` 指令即可運行。對安全管控極嚴格、禁止下載任何執行檔的客戶電腦非常有用。
* **劣勢**：高度依賴第三方 SSH 伺服器的存活率與頻寬。
* **快速命令 (Serveo)**：
  ```bash
...
#### H. Tailscale Funnel -【安全網格的對外窗口】
* **介紹**：基於 WireGuard 的虛擬網卡安全網路（Tailscale）推出的對外暴露功能。
* **優勢**：整合於 Tailscale 安全網格中，非常安全，且能自動獲取 Let's Encrypt HTTPS 憑證。
* **劣勢**：需要在本地啟用 Tailscale 並加入節點，且機器需被授權啟用 Funnel 功能。

#### I. Pinggy -【基於 SSH 的超簡易 Tunnel】
* **介紹**：一個專注於提供極簡 SSH 通道的付費/免費服務。
* **優勢**：與 serveo 類似，免安裝，支援實時 Web 介面查看連線封包，對前端 debug 非常實用。
* **劣勢**：免費版連線有時間限制，且網址會隨機變化。
* **快速命令**：
  ```bash
  ssh -t a:localhost:8000+ssl@connect.pinggy.io
  ```

---

### 3. 課堂實戰思考：轉 Port 技術在資安防護與白名單檢測中的雙刃劍效應

在課堂中，Force 老師可以引導學生從資安架構師的角度思考以下問題：

* **資安威脅：防禦規避 (Defense Evasion)**
  企業內網通常會透過防火牆限制員工訪問外部網站，但「內網穿透」工具（特別是如 `SSH 反向隧道` 或 `cloudflared`）其流量通常會偽裝在標準的 HTTPS (Port 443) 或 SSH (Port 22) 之中，這使得企業的傳統防火牆極難察覺。惡意軟體或內部人員極易利用這些工具將敏感資料洩露至公網，或是建立一個反彈 Shell 回連通道。
* **環境檢測的侷限性**
  當我們使用 FALO 的 `demo.html` 進行網絡檢測時，如果客戶端電腦同時運行了內網穿透軟體或企業 VPN，檢測到的連通性可能並非「真實的物理網絡放行狀態」，而是「穿透通道內的狀態」。因此，顧問在解讀報告時，必須確認客戶測試時是否關閉了不必要的 VPN 或 Proxy，以確保數據的真實性。
